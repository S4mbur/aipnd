...
def train_model(image_datasets, arch='vgg19', hidden_units=4096, epochs=25, learning_rate=0.001, gpu=False, checkpoint=''):
    ...
    model = load_custom_model(architecture=architecture, num_labels=num_labels, hidden_units=hidden_units)
...
if args.data_dir:
    ...
    image_datasets = {
        dataset_type: datasets.ImageFolder(root=args.data_dir + '/' + dataset_type, transform=data_transforms[dataset_type])
        for dataset_type in data_transforms.keys()
    }

    trained_model = train_model(image_datasets)
